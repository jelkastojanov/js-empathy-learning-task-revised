...
jsPsych.plugins["serial-reaction-time-mouse"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      grid_lion_size: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Grid lion size',
        default: 100,
        description: 'The width and height in pixels of each lion in the grid.'
      },
      target_color: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Target color',
        default: "#999",
        description: 'The color of the target lion.'
      },
      response_ends_trial: {
        type: jsPsych.plugins.parameterType.BOOL,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // display stimulus
    var stimulus = this.stimulus(trial.grid, trial.grid_lion_size);
  ...
  plugin.stimulus = function(grid, lion_size, target, target_color, labels) {
    var stimulus = "<div id='jspsych-serial-reaction-time-stimulus' style='margin:auto; display: table; table-layout: fixed; border-spacing:"+lion_size/4+"px'>";
    for(var i=0; i<grid.length; i++){
      ...
      for(var j=0; j<grid[i].length; j++){
        ...
        stimulus += "<div class='"+classname+"' id='jspsych-serial-reaction-time-stimulus-cell-"+i+"-"+j+"' "+
          ...
          "style='width:"+lion_size+"px; height:"+lion_size+"px; display:table-cell; vertical-align:middle; text-align: center; cursor: pointer; font-size:"+lion_size/2+"px;";
        if(grid[i][j] == 1){
